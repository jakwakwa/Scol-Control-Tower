...
3. **Test schema**: `dev:browser-flow` runs `db:migrate:test` (pending migrations on test Turso). Full reset: `bun run test:db:reset` (drop all + migrate) or `bun run db:reset` (dev + test when `.env.test` has `TEST_DATABASE_URL`). Ad-hoc schema sync without migration files: `db:push:test`.
