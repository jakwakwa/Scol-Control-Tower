...
async function markQuoteApproved(workflowId: number) {
		...
		(
			await db
				...
				.values({
					...
					rationale: "Scenario fallback quote generated by mock-scenario-runner",
					details: JSON.stringify({
						source: "mock_scenario_runner",
						fallback: true,
	...
	if (!targetQuote) {
		throw new Error(`Unable to create fallback quote for workflow ${workflowId}`);
	}

	await db
		.update(quotes)
		.set({ status: "pending_signature", updatedAt: new Date() })
		.where(eq(quotes.id, targetQuote.id));
...
async function approveDecisionForm(params: {
	...
	const existingSubmission = await db
		...
		.where(eq(applicantSubmissions.applicantMagiclinkFormId, targetForm.id))
		.limit(1);

	if (existingSubmission.length === 0) {
		await recordFormSubmission({
			applicantMagiclinkFormId: targetForm.id,
			applicantId: params.applicantId,
			workflowId: params.workflowId,
			formType: params.formType,
			data: params.data,
	...
	await recordFormDecision({
		formInstanceId: targetForm.id,
		outcome: "APPROVED",
		reason: `Approved by ${MOCK_ACTOR}`,
	});

	const [refreshedForm] = await db
		.select()
		.from(applicantMagiclinkForms)
		.where(eq(applicantMagiclinkForms.id, targetForm.id))
		.limit(1);

	if (!refreshedForm) {
		throw new Error(`Form ${targetForm.id} not found after decision update`);
	}
...
export const mockScenarioRunner = inngest.createFunction(
	...
	async ({ event, step }) => {
		...
		await step.run("sign-quote", async () => {
			...
			// Skip programmatic sign when the external user has already submitted the form.
			// The user will approve via the form UI; we must not race and pre-empt their decision.
			const [form] = await db
				.select()
				.from(applicantMagiclinkForms)
				.where(
					and(
						eq(applicantMagiclinkForms.workflowId, event.data.workflowId),
			...
			if (form?.status === "submitted") {
				const [submission] = await db
					.select({ submittedBy: applicantSubmissions.submittedBy })
					.from(applicantSubmissions)
					.where(eq(applicantSubmissions.applicantMagiclinkFormId, form.id))
					.limit(1);
				if (submission?.submittedBy && submission.submittedBy !== MOCK_ACTOR) {
					return; // User submitted; they will approve. Do not race.
				}
			}
