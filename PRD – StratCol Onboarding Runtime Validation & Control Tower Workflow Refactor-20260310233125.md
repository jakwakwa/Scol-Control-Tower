...
# aPRD – StratCol Onboarding Runtime Validation & Control Tower Workflow Refactor
## 1\. Overview / Summary
...
2. **Refactor of** **`ControlTowerWorkflow.ts`** **into a modular composite architecture**, reducing cognitive load, enabling safer itera tion, and supporting the timeout / kill-switch behaviors required by (1).
...
*   **Status model & dashboard UX**: The new `TERMINATED` state should be surfaced in a new dedicated column in the workflow dashboard table rows (e.g., a "Termination Reason / State" column) alongside existing statuses like `PENDING`, `RUNNING`, and `TIMEOUT`. This column should make it obvious when a run has been terminated by the guards vs still in progress or timed out.
...
### 7\. Validation & ingest perimeter clarifications
**Canonical schemas for** **`onboarding/lead.created`** **and external sanctions events**:
The canonical field definitions (types, required/optional fields, enums) will be defined by the implementing developer as part of a follow-on technical design.
These are **TBD in technical design**, not set by product.

**No legacy payload support**:
This project is **not yet in production**; no legacy or ad-hoc payload variants will be supported.
All Zod schemas should represent the new canonical contract.
Remove all legacy remnants and variants from validation logic.
**Strict validation only**:
There is **no requirement for soft-validation** or warning-only behavior for legacy payloads
Malformed or non-conforming payloads will fail hard at the perimeter.
The overall owner is a **single developer (project owner)** working with AI assistants, responsible for Zod schemas and their evolution.

###   

### 7.3 Modular workflow architecture
**Module structure**
*   Split `ControlTowerWorkflow.ts` into stage modules
...
*   **Migration of existing** **`TIMEOUT`** **runs**: We must migrate all existing `TIMEOUT` runs to `TERMINATED` with a reason code where applicable, so that new semantics apply retroactively and not just to new runs.

## 8\. Risks, Assumptions & Dependencies
*   **Schema strictness risk**: Strict schemas may surface previously hidden inconsistencies during integration, but this is acceptable given the non-production status; the owner accepts these failures as part of hardening.
*   **Timeout configuration**: The 14-day facility application timeout and 30-day approval window remain the defaults for this iteration. Timeouts should be designed to be configurable per product/segment in the future, but this is a backlog/non-urgent item. The current scope is to keep current values while ensuring the design does not block future configurability.
*   **Terminated workflow re-activation**: When a run times out and is terminated, it must always start fresh; there is no re-activation path for `TERMINATED` workflows. Re-use must happen via a new workflow run. This constraint simplifies reasoning and audit trails.
*   **One-time migration**: The one-time migration from `TIMEOUT` to `TERMINATED` is in scope, but does **not** imply complex re-processing of workflows.
*   **Single-owner governance**: Single-owner governance can be fast but increases the risk of implicit decisions. It is recommended to document schema changes in seperate [schema-changes.md](http://schema-changes.md)
*   **Architecture phasing**: The near-term implementation should focus on a thin traffic-director pattern (lean orchestrator that coordinates stage modules). The longer-term direction is to evolve toward an event-driven model where stages emit `stageN.completed` events that trigger subsequent stages independently. The exact phasing and which stages go event-driven first will be refined in a later technical design. There are no platform constraints (event bus, latency, cost) currently blocking either pattern.
*   **Rollout strategy**: The preferred rollout strategy is to use environment-flagged and/or canary/product-slice deployments for both validation and timeout changes (e.g., start in non-prod, then a subset of customers or products in prod). This is confirmed and is intended to minimize risk while still allowing detection of unexpected validation failures. No separate bulk clean-up/backfill job is required for zombie workflows beyond the one-time status migration from `TIMEOUT` to `TERMINATED`.
### 8.1 Risks
...
## 10\. Open Questions
Private ([https://app.clickup.com/t/869cce3hy](https://app.clickup.com/t/869cce3hy))
### Key Decisions & Assumptions
*   Canonical schemas will be defined by the developer during implementation; no legacy compatibility layer is required.
*   Strict schemas will be enforced at the perimeter; malformed payloads will fail hard. No soft-validation or warning-only handling.
*   Timeouts remain 14 days (facility) and 30 days (approval) for this iteration; making timeouts configurable is a backlog item, but the design must not block future configurability.
*   Terminated workflows cannot be re-activated; new runs must start fresh.
*   Migration of existing `TIMEOUT` runs to `TERMINATED` with a reason code is in scope.
*   Dashboard UX will surface `TERMINATED` in a dedicated column.
*   Rollout will use environment-flagged/canary/product-slice deployments; no bulk clean-up/backfill job beyond the status migration is required.
*   Ownership of Zod schemas and breaking change approvals is with the single developer/project owner, tracked via Private ([https://app.clickup.com/t/869ccebdp](https://app.clickup.com/t/869ccebdp)).
*   Architecture will follow a phased approach: thin traffic-director now, event-driven evolution later.
### Still Open / To Be Determined
*   What is the precise phasing plan for moving from thin traffic-director to fully event-driven per stage, and which environments/products migrate first?
*   Any further UX details for dashboard display of `TERMINATED` or migration edge cases (if discovered).