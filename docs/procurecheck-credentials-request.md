...
- **Option A:** “Username: `something@domain.com`, Password: `••••••••`”  
  → Use those exactly in your `.env` as `PROCURECHECK_USERNAME` and `PROCURECHECK_PASSWORD`.
...
- **Option C:** “We’ve created an API user; credentials are in the portal.”  
  → Ask them to send the username and password in a secure way (e.g. separate from the email, or via your secure channel). You need the actual values for your `.env`.
...
2. Run the verification script from the repo root:
   ```bash
   ./scripts/verify-procurecheck-credentials.sh
   ```
3. If you see `OK: ProcureCheck credentials are valid (JWT received).`, you’re done.
