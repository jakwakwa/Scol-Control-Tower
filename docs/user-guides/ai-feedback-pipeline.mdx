...
description: "How human adjudications are captured as structured data to retrain AI risk models."
---
...
The pipeline follows a simple principle: **every adjudication becomes a retrainable data point**.
...
The adjudication taxonomy is a structured classification system that maps human adjudications into programmatic categories. It is defined in `lib/constants/adjudication-taxonomy.ts`.
...
| `POLICY_EXCEPTION` | Human adjudication based on policy or management decision |
...
- **Policy Exception**: Management Exception, New Regulatory Guidance, Client Tier Exception
...
| Field | Description |
| :-- | :-- |
| `aiOutcome` | What the AI recommended (APPROVE, MANUAL\_REVIEW, DECLINE) |
| `aiConfidence` | AI confidence score (0–100) |
| `aiCheckType` | Type of AI check (identity\_verification, document\_analysis, risk\_screening, aggregated) |
| `humanOutcome` | What the human decided (APPROVED, REJECTED, REQUEST\_MORE\_INFO) |
| `adjudicationReason` | Selected category from the taxonomy |
| `adjudicationDetail` | Optional detail or subcategory |
| `adjudicationNotes` | Optional free text (max 300 characters) |
...
Both risk decision endpoints capture structured adjudication data:
...
<Badge color="yellow" size="lg" shape="pill" stroke icon="alert-triangle">outdated snippet</Badge>
...
```json
{
  ...
  "decision": {
    "outcome": "APPROVED",
    "adjudicationReason": "MISSING_CONTEXT",
    "decidedBy": "user_abc"
  },
  "feedback": {
    "feedbackLogId": 456,
