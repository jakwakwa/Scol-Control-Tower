---
title: "Risk review"
description: "Review AI risk assessments, make structured adjudication decisions, and feed the AI retraining pipeline."
---

## Risk review queue

Open **Risk Review** from the sidebar to see all workflows awaiting a human risk decision. Items arrive here when the AI risk assessment flags a workflow for manual review.

### What you see

Each card in the queue shows:

- **Client name** and workflow ID
- **Company name** and entity type
- **AI trust score** (0–100)
- **AI recommendation** (e.g. Approve, Manual Review, Decline)
- **Risk level** badge (Low, Medium, High, Critical)
- **Check-level status context** (e.g. ITC manual-required, validation fallback) when checks completed in degraded or partial-failure mode
- **Vendor-failure context** showing whether a check is in `manual_required`, `failed`, `paused`, or advisory/unavailable mode
- **Document-processing context** indicating when Stage 3 bank-statement risk analysis used native PDF document understanding versus degraded/manual-required fallback
- **Time waiting** since the item entered the queue

### Quick actions

From every card you can:

- **View Details** to open the full risk review sheet
- **Approve** or **Reject** to open the structured decision dialog

## Structured decision dialog

When you approve or reject a workflow, the system captures structured adjudication data instead of free-text reasons. This data feeds the AI retraining pipeline automatically.

### Required fields

1. **Adjudication Category** — select the category that best describes your assessment. Options include:

| Category | When to use |
| :--- | :--- |
| AI Decision Aligned | Your decision matches the AI recommendation |
| Missing Context | The AI lacked information you have |
| Incorrect Risk Scoring | The AI scored too high or too low |
| False Positive Flag | The AI flagged something that is not a real issue |
| False Negative Miss | The AI missed a real risk factor |
| Policy Exception | You are adjudicating based on a policy or management decision |
| Data Quality Issue | The AI used stale, incorrect, or incomplete data |
| Other | None of the above — additional details are required |

2. **Subcategory** (optional) — a more specific label within the selected category. For example, under "False Positive Flag" you might select "Name Collision (Not Same Entity)".

3. **Additional Notes** — free text for extra context. Required when the category is "Other".

### AI divergence warning

When your decision contradicts the AI recommendation (e.g. approving when the AI said "Decline"), the dialog shows an **AI Divergence** warning. This flags the decision for priority retraining review.

### Validation rules

- A category is always required.
- Rejections cannot use "AI Decision Aligned" (since declining contradicts alignment).
- "Other" requires additional notes.

## Risk review detail sheet

Select **View Details** to open the full risk review sheet for an item.

### Tabs

- **Overview**: applicant details, AI trust score, entity information, risk flags
- **Risk Assessment**: full AI risk analysis, sanctions check results, validation summary
- **Documents**: uploaded documents with verification status (bank statements, accountant letters, etc.)
- **Timeline**: audit log of all workflow events and transitions

### Quick approve/reject from detail view

The detail view includes approve and reject buttons. These use default categories:

- **Approve** defaults to "AI Decision Aligned"
- **Reject** defaults to "Other"

For full structured input, use the decision dialog from the queue cards instead.

## Review model

Risk Review now uses a **single human decision gate** for this cycle:

- Stage 3 runs procurement, ITC/sanctions, and AI analysis in parallel
- The Reporter Agent synthesizes all available results (including degraded/manual-required outputs)
- A single Risk Manager decision is captured in Stage 4

The procurement section on the detail view shows real ProcureCheck results: vendor details, overall category summary, and per-category check tables (CIPC, Property, Restricted List, Legal, SAFPS, Persal). Loading skeletons display while checks are running; an error card appears if the check failed or requires manual review. Procurement is not a separate blocking decision queue. While items are awaiting your decision, the system sends **reminders** and **does not auto-terminate** the workflow on review timeout.

### Interpreting degraded vendor states

When automation cannot complete safely, Risk Review should treat the check-level state as the source of truth:

- **`manual_required`** means the workflow kept moving, but a human still needs to complete or confirm that check before approval.
- **`failed`** means the automation ended unsuccessfully without a full manual-review envelope. Treat these items as needing extra attention, especially if no matching workflow notification exists.
- **`paused`** means the workflow is intentionally stopped and cannot progress until the required decision is recorded. The sanctions adjudication path is the main example.
- **Advisory unavailable / inconclusive** states, such as VAT evidence issues, should inform your decision but should not be treated as hard blockers on their own.

## What happens after a decision

1. The decision is logged as a `workflow_event` in the database.
2. A **structured feedback log** is created in `ai_feedback_logs` for retraining (see [AI feedback pipeline](/user-guides/ai-feedback-pipeline)).
3. If the decision diverges from the AI recommendation, an `ai/feedback.divergence_detected` event is emitted.
4. The workflow resumes or terminates based on the outcome.
5. If the decision is a rejection, the workflow terminates and downstream processing is halted.
